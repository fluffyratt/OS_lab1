...
set "SERVER=%BIN%\ServerBlocking.exe"
set "CLIENT=%BIN%\ClientBlocking.exe"
set "OUT=%~dp0results_blocking.csv"

echo impl,transport,mode,size,conns,count,secs,rate_per_s,mb_per_s,X_us,Y_us,Z_us,failed>"%OUT%"

echo ========================================
echo BLOCKING SOCKET BENCHMARK
echo ========================================
...
taskkill /F /IM ServerBlocking.exe >nul 2>&1
...
"%CLIENT%" -a %~1 -m %~2 -s %~3 -n %~4 -c %~5 > tmp_out.txt

type tmp_out.txt

for /f "tokens=1,* delims=," %%A in ('findstr /B "CSV," tmp_out.txt') do (
    echo %%B>>"%OUT%"
)
